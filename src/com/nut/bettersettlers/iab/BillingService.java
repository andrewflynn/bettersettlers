...
import com.nut.bettersettlers.iab.Security.VerifiedPurchase;
...
public class BillingService extends Service implements ServiceConnection {
    ...
    public abstract class BillingRequest {
        ...
        protected Bundle makeRequestBundle(String method) {
            ...
            request.putString(IabConsts.BILLING_REQUEST_METHOD, method);
            request.putInt(IabConsts.BILLING_REQUEST_API_VERSION, 1);
            request.putString(IabConsts.BILLING_REQUEST_PACKAGE_NAME, getPackageName());
            return request;
        }
    ...
    public class RequestPurchase extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            request.putString(IabConsts.BILLING_REQUEST_ITEM_ID, mProductId);
            // Note that the developer payload is optional.
            if (mDeveloperPayload != null) {
                request.putString(IabConsts.BILLING_REQUEST_DEVELOPER_PAYLOAD, mDeveloperPayload);
            }
            Bundle response = mService.sendBillingRequest(request);
            ...
            return response.getLong(IabConsts.BILLING_RESPONSE_REQUEST_ID,
                    IabConsts.BILLING_RESPONSE_INVALID_REQUEST_ID);
        }
    ...
    public class ConfirmNotifications extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            request.putStringArray(IabConsts.BILLING_REQUEST_NOTIFY_IDS, mNotifyIds);
            Bundle response = mService.sendBillingRequest(request);
    ...
    public class GetPurchaseInformation extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            request.putLong(IabConsts.BILLING_REQUEST_NONCE, mNonce);
            request.putStringArray(IabConsts.BILLING_REQUEST_NOTIFY_IDS, mNotifyIds);
            Bundle response = mService.sendBillingRequest(request);
    ...
    public void handleCommand(Intent intent, int startId) {
        ...
        } else if (IabConsts.ACTION_GET_PURCHASE_INFORMATION.equals(action)) {
            String notifyId = intent.getStringExtra(IabConsts.NOTIFICATION_ID);
            getPurchaseInformation(startId, new String[] { notifyId });
        } else if (IabConsts.ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
            String signedData = intent.getStringExtra(IabConsts.INAPP_SIGNED_DATA);
            String signature = intent.getStringExtra(IabConsts.INAPP_SIGNATURE);
            purchaseStateChanged(startId, signedData, signature);
        } else if (IabConsts.ACTION_RESPONSE_CODE.equals(action)) {
            long requestId = intent.getLongExtra(IabConsts.INAPP_REQUEST_ID, -1);
            int responseCodeIndex = intent.getIntExtra(IabConsts.INAPP_RESPONSE_CODE,
     ...
     * receives an intent with the action {@link IabConsts#ACTION_NOTIFY}.
     ...
     * in an intent with the action {@link IabConsts#ACTION_PURCHASE_STATE_CHANGED}.
